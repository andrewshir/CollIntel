...
from tree_image import draw_tree
...
def load_titanic(csv_file='train.csv'):
    ...
    df = pd.read_csv(work_path + csv_file, index_col=None, header=True, names=col_names)
    print df
    arr = df.values
    return arr
...
def run_decision_tree(train_data, test_data=None, submit_data=None):
    ...
    draw_tree(dt.tree_, work_path+'tree.jpg')
...
#load train data
# X = load_titanic()
# X = load_titanic("train_men_3d.csv")
X = load_titanic(csv_file="train_tree.csv")
X_shape = np.shape(X)

# split data set
# n_sample = X_shape[0]
# latest = int(n_sample * 0.9)
